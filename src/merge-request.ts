...
import { MergeRequestData, CodeDiff, ApiResponse, User } from "./types";
...
export class MergeRequestFetcher {
  ...
  public async fetchAllMerged(
    ...
    try {
      while (
        hasMore &&
        (!options.maxResults || mergeRequests.length < options.maxResults)
      ) {
        const response = await this.client.getMergedMergeRequests(
          page,
          perPage,
          {
        ...
        for (const mr of mrs) {
          const diffResponse = await this.client.getMergeRequestDiff(mr.iid);
          console.log(diffResponse);
          if (!diffResponse?.data) {
            throw new Error("Invalid API response: missing diff data");
          }
          const changes: CodeDiff[] = Array.isArray(diffResponse.data)
            ? diffResponse.data.map((change: any) => ({
          ...
          if (
            options.maxResults &&
            mergeRequests.length >= options.maxResults
          ) {
            hasMore = false;
            break;
          }
